...
//	{600*KHZ, 480*KHZ, 400*KHZ},
	{800*KHZ, 480*KHZ, 400*KHZ},
//	{650*KHZ, 480*KHZ, 400*KHZ},//shanghai
//	{700*KHZ, 640*KHZ, 400*KHZ},
//	{750*KHZ, 640*KHZ, 400*KHZ},
//	{800*KHZ, 640*KHZ, 400*KHZ},//shanghai
};
...
//copy sep_freq_table to local structure sep_freq_curr_limit_table
	...
	if (freqs.old == freqs.new) {
		freq_dbg("freqs.old == freqs.new = %d     target_freq = %d\n", freqs.new, target_freq);
		return ret;
	}

	cpufreq_notify_transition(&freqs, CPUFREQ_PRECHANGE);

	freq_dbg("cpufreq: changing clk to %ld; target = %ld, oldfreq = %ld\n",freqs.new, target_freq, freqs.old);
	freq_dbg("cpufreq: apll_clk = %ld;    mpll_clk = %ld, dpll_clk = %ld\n",sep_freq_curr_limit_table.apll_freq, sep_freq_curr_limit_table.mpll_freq, sep_freq_curr_limit_table.dpll_freq);
