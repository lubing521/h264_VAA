...
INSTANCE InitInstanceData = 
{
        ...
        0,// bAudioChannelSwap
        B_AUD_EN_I2S0 | B_AUD_I2S | M_AUD_16BIT, // bAudioChannelEnable, 0x01, REG[0xE0], defined in cat6613_drv.h
...
//    if (szID[0] == 0x00 && szID[1] == 0xCA && szID[1] == 0x13 && szID[1] == 0x06) szID[0] ???
    if (szID[1] == 0xCA && szID[2] == 0x13 && szID[3] == 0x06){
        bPass = TRUE;
        printk("TX Chip Revision ID: %d\n", szID[0]);     
    }else{
        printk("NG, Read TX Chip ID:%02X%02X%02X%02Xh (expected:00CA1306h)\n", szID[0], szID[1], szID[2], szID[3]);     
    }
    printk("bPass = %d\n", (int)bPass);                
    return bPass;
}
...
bool HDMITX_Init(void){
    ...
    //    printk("Failed to find CAT6613 HDMI-TX Chip.\n");
    //    bSuccess = FALSE;
    //}    
...
void HDMITX_SetAVIInfoFrame(unsigned char VIC, unsigned char OutputColorMode, bool b16x9, bool ITU709)
{
    AVI_InfoFrame AviInfo;
    BYTE pixelrep = 0;
    
    printk("HDMITX_SetAVIInfoFrame, VIC=%d, ColorMode=%d, Aspect-Ratio=%s, ITU709=%s\n",
...
/*bool HDMITX_DevLoopProc()
{
    ...
    if( HPDChange )
    {
        ...
        if( HPD )
        {
            ...
            {
                OS_printk("Invalid HDMI Display\n");
                bHDMIMode = FALSE ;
                bAudioEnable = FALSE ;
            }

            OS_printk("HDMITX_SetOutput\n");
            //HDMITX_SetOutput() ;
