...
struct i2c_client* gHdmiClient = NULL;

SYS_STATUS HDMITX_WriteI2C_Byte(BYTE RegAddr, BYTE val)
{
	//printk("write to %02X with value: %02X\n",RegAddr, val);
	return i2c_smbus_write_byte_data(gHdmiClient, RegAddr, val);
}
...
BYTE HDMITX_ReadI2C_Byte(BYTE RegAddr)
{
	...
	printk("address = 0x%x; ", RegAddr);
	status = i2c_smbus_read_byte_data(gHdmiClient, RegAddr);
	printk("value = 0x%x\n", status);
...
//    for(i=0x10;i<=0x19;i++){
    for(i=0;i< 256;i++){
        ...
        printk("TX Reg[%02Xh] = %02Xh\n", i, data);
        mdelay(20);  // wait uart dump finish
    }        
}
//////////////////////////////////////////////////////////////////////
...
static BYTE InitCAT6613_HDCPROM()
{
    /*BYTE uc[5]  ;
...
BOOL EnableVideoOutput(VIDEOPCLKLEVEL level,BYTE inputColorMode,BYTE outputColorMode,BYTE bHDMI)
{
    ...
    printk("EnableVideoOutput\n");
    HDMITX_WriteI2C_Byte(REG_TX_SW_RST,B_VID_RST|B_AUD_RST|B_AREF_RST|B_HDCP_RST) ; //2011-6-20
...
BOOL EnableAudioOutput(ULONG VideoPixelClock,BYTE bAudioSampleFreq,BYTE ChannelNumber,BYTE bAudSWL,BYTE bSPDIF)
{
  ...
  // richard   unsigned long N ;
    ...
    */
	...
	//HDMITX_WriteI2C_Byte(REG_TX_SW_RST,~(B_AUD_RST|B_AREF_RST)) ;2011-6-20
    SetAudioFormat(ChannelNumber,bAudioChannelEnable,bAudioSampleFreq,bAudSWL,bSPDIF) ;
	HDMITX_WriteI2C_Byte(REG_TX_SW_RST,~(B_AUD_RST|B_HDMI_RST|B_VID_RST|B_AREF_RST|B_REF_RST));
    #ifdef HDMITX_REG_DEBUG
    DumpCatHDMITXReg() ;
...
CheckHDMITX(BYTE *pHPD,BYTE *pHPDChange)
{
  ...
  //  OS_printk("sysstat(REG[0x0E])=%02Xh\r\n", sysstat);    
    ...
    if(sysstat & B_INT_ACTIVE)  // interrupt is activce
    {
		...
		if(intdata1 & B_INT_DDC_BUS_HANG)
		{
            ...
            {
                printk("when DDC hang,and aborted DDC,the HDCP authentication need to restart.\n") ;
#ifdef SUPPORT_HDCP
...
ProgramSyncEmbeddedVideoMode(BYTE VIC,BYTE bInputSignalType)
{
    ...
    printk("ProgramSyncEmbeddedVideoMode(%d,%x)\n",VIC,bInputSignalType) ;
...
SetInputMode(BYTE InputMode,BYTE bInputSignalType)
{
    ...
    {
        ucData |= B_PCLKDIV2 ; printk("PCLK Divided by 2 mode\n") ;
    }
...
{
	//write_data(REG_TX_SW_RST,          B_AUD_RST|B_AREF_RST|B_HDCP_RST) ;
    ...
    mdelay(1) ;
    HDMITX_WriteI2C_Byte(REG_TX_SW_RST,B_VID_RST|B_AREF_RST|B_AUD_RST|B_HDCP_RST) ;
    mdelay(100) ;
    HDMITX_WriteI2C_Byte(REG_TX_SW_RST,          B_AREF_RST|B_AUD_RST|B_HDCP_RST) ;
...
SetAudioFormat(BYTE NumChannel,BYTE AudioEnable,BYTE bSampleFreq,BYTE AudSWL,BYTE AudioCatCode)
{
    ...
    printk("SetAudioFormat(%d channel,%02X,SampleFreq %d,AudSWL %d,%02X)\n",NumChannel,AudioEnable,bSampleFreq,AudSWL,AudioCatCode) ;
...
//richard remove    Instance[0].bOutputAudioMode |= 0x41 ;
    ...
    //HDMITX_WriteI2C_Byte(REG_TX_SW_RST, B_AUD_RST|B_AREF_RST) ;
    HDMITX_WriteI2C_Byte(REG_TX_AUDIO_CTRL1,Instance[0].bOutputAudioMode) ; // regE1 bOutputAudioMode should be loaded from ROM image.
...
//     Switch_HDMITX_Bank(0) ;
    ...
    Switch_HDMITX_Bank(1) ;
    HDMITX_WriteI2C_Byte(REG_TX_AUDCHST_MODE,0 |((NumChannel == 1)?1:0)) ; // 2 audio channel without pre-emphasis,if NumChannel set it as 1.
    //HDMITX_WriteI2C_Byte(REG_TX_AUDCHST_CAT,AudioCatCode) ; 2011-6-20
    HDMITX_WriteI2C_Byte(REG_TX_AUDCHST_SRCNUM,SoruceNum) ;
...
SetNCTS(ULONG PCLK,ULONG Fs)
{
    ...
    HDMITX_WriteI2C_Byte(REG_TX_CLK_CTRL0,(~M_EXT_MCLK_SEL) | B_EXT_256FS) ;
    return ER_SUCCESS ;
}
...
AbortDDC()
{
    ...
    for( timeout = 0 ; timeout < 200 ; timeout++ )
    {
        ...
        {
            printk("AbortDDC Fail by reg16=%02X\n",uc) ;
            break ;
        }
...
HDCP_GetKSVList(BYTE *pKSVList,BYTE cDownStream)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_ERROR)
        {
            printk("HDCP_GetKSVList(): DDC Fail by REG_TX_DDC_STATUS = %x.\n",ucdata) ;
            return ER_FAIL ;
        }
        DelayMS(5) ;
    }
...
HDCP_GetVr(BYTE *pVr)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_ERROR)
        {
            printk("HDCP_GetVr(): DDC fail by REG_TX_DDC_STATUS = %x.\n",ucdata) ;
            return ER_FAIL ;
        }
        DelayMS(5) ;
    }
    ...
    if(TimeOut == 0)
    {
        printk("HDCP_GetVr(): DDC fail by timeout.\n",ucdata) ;
        return ER_FAIL ;
    }
...
SetAVIInfoFrame(AVI_InfoFrame *pAVIInfoFrame)
{
	...
	DISABLE_AVI_INFOFRM_PKT();
    Switch_HDMITX_Bank(1) ;
    HDMITX_WriteI2C_Byte(0x70,0x00);
	...
	//printk("SetAVIInfo(): ") ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB1)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB2)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB3)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB4)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB5)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB6)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB7)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB8)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB9)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB10)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB11)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB12)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB13)) ;
	//printk("\n") ;
    ucData -= 0x80+AVI_INFOFRAME_VER+AVI_INFOFRAME_TYPE+AVI_INFOFRAME_LEN ;
	HDMITX_WriteI2C_Byte(REG_TX_AVIINFO_SUM,0x35);
