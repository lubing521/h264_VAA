...
#include "hdmitx.h"
#include "cat6613_sys.h" // richard add
#include <mach/hdmi.h>
//#include "edid.h" // richard add

//extern int TX_HDP;  // richard add
enum ColorDepth gColorDepth = HDMI_CD_24;
...
_XDATA BYTE VIC = HDMI_720p60; // 720p60
_XDATA BYTE pixelrep ; // no pixelrepeating
_XDATA HDMI_Aspec aspec ;
_XDATA HDMI_Colorimetry Colorimetry ;

BOOL bHDMIMode, bAudioEnable ;  //Defined in the function of HDMITX_DevLoopProc , not used now 2011-6-9.

//About audio
BYTE gAudioSampleFreq = SF_44KHZ;
BYTE gAudioChannelNum = CH_2;
BYTE gAudioWordLen = WORD_16;
...
HDMITX_SetOutput()
{
    ...
    ConfigAVIInfoFrame(VIC, pixelrep) ;//2011-6-20
    if( bHDMIMode )
    {
    	printk("ConfigAVIInfoFrame, VIC=%d\n", VIC);
        //ConfigAVIInfoFrame(VIC, pixelrep) ;//2011-6-20
#ifdef SUPPORT_HDCP
        EnableHDCP(TRUE) ;
#endif
		//bAudioEnable = 0;
		//if( bAudioEnable )//2011-6-20-----------------------------------------
		//{
        ...
        bool bSPDIF = FALSE;
        //EnableAudioOutput(TMDSClock,AUDFS_44p1KHz, 2, 16, bSPDIF);  // richard modify
		EnableAudioOutput(TMDSClock, gAudioSampleFreq, gAudioChannelNum, gAudioWordLen, bSPDIF);  // richard modify
        ConfigAudioInfoFrm() ;
		//}//2011-6-20----------------------------------------------------------
    }
    SetAVMute(FALSE) ;
...
ConfigAudioInfoFrm()
{
    ...
    //AudioInfo.pktbyte.AUD_DB[0] |= 1;
    //AudioInfo.pktbyte.AUD_DB[1] |= 2<<2;
    switch(gAudioChannelNum)
	{
		case CH_2:
			AudioInfo.pktbyte.AUD_DB[0] |= 1;
			break;
		case CH_3:
			AudioInfo.pktbyte.AUD_DB[0] |= 2;
			break;
		case CH_4:
			AudioInfo.pktbyte.AUD_DB[0] |= 3;
			break;
		case CH_5:
			AudioInfo.pktbyte.AUD_DB[0] |= 4;
			break;
		case CH_6:
			AudioInfo.pktbyte.AUD_DB[0] |= 5;
			break;
		case CH_7:
			AudioInfo.pktbyte.AUD_DB[0] |= 6;
			break;
		case CH_8:
			AudioInfo.pktbyte.AUD_DB[0] |= 7;
			break;
	}	
	
	switch(gAudioSampleFreq)
	{
		case SF_32KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 1<<2;
			break;
		case SF_44KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 2<<2;
			break;
		case SF_48KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 3<<2;
			break;
		case SF_88KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 4<<2;
			break;
		case SF_96KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 5<<2;
			break;
		case SF_176KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 6<<2;
			break;
		case SF_192KHZ:
			AudioInfo.pktbyte.AUD_DB[1] |= 7<<2;
			break;
	}
