...
static int sr_read(struct usbnet *dev, u8 reg, u16 length, void *data)
{
	...
	devdbg(dev, "sr_read() reg=0x%02x length=%d", reg, length);
...
static int sr_write(struct usbnet *dev, u8 reg, u16 length, void *data)
{
	...
	devdbg(dev, "sr_write() reg=0x%02x, length=%d", reg, length);
...
{
	devdbg(dev, "sr_write_reg() reg=0x%02x, value=0x%02x", reg, value);
...
static void sr_write_async_helper(struct usbnet *dev, u8 reg, u8 value, u16 length, void *data)
{
	...
	if (!urb) {
		deverr(dev, "Error allocating URB in sr_write_async_helper!");
		return;
	}
...
{
	devdbg(dev, "sr_write_async() reg=0x%02x length=%d", reg, length);
...
{
	devdbg(dev, "sr_write_reg_async() reg=0x%02x value=0x%02x", reg, value);
...
//fgs+++
	...
	return rc;
//fgs---
//fgs	return mii_link_ok(&dev->mii);
}
...
static struct ethtool_ops sr9700_android_ethtool_ops = {
...
static void sr9700_android_set_multicast(struct net_device *net)
{
	...
	if (net->flags & IFF_PROMISC) {
		rx_ctl |= 0x02;
	} else if (net->flags & IFF_ALLMULTI || net->mc_count > SR_MCAST_MAX) {
		rx_ctl |= 0x04;
...
static const struct net_device_ops sr9700_netdev_ops = {
	...
	.ndo_set_mac_address	= sr9700_android_set_mac_address,//fgs add from 2818
};
...
//fgs	dev->net->netdev_ops->ndo_do_ioctl = sr9700_android_ioctl;
//fgs	dev->net->netdev_ops->ndo_set_multicast_list = sr9700_android_set_multicast;
...
static const struct driver_info sr9700_android_info = {
	.description	= "Supereal SR9700_ANDROID USB Ethernet",
	.flags		= FLAG_ETHER,
...
MODULE_DESCRIPTION("Supereal SR9700 one chip USB 2.0 ethernet devices on android platform");
MODULE_LICENSE("GPL");
