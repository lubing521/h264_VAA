...
ATH_DEBUG_INSTANTIATE_MODULE_VAR(driver,
                                 ...
                                 ATH_DEBUG_MASK_DEFAULTS | ATH_DEBUG_WLAN_SCAN |ATH_DEBUG_WLAN_CONNECT |
...
int debugdriver = 0;
unsigned int debughtc = 128;// 0;
unsigned int debugbmi = 1;
unsigned int debughif = 2;//0;
...
int wifi_suspend(int state)
{
    ...
    printk("wifi_suspend enter\n");

    if( ar == NULL)
	    return 0;

     /* if (ar->arConnected == TRUE) { */
     /* 	     printk("wifi in connect state, reject wifi_suspend\n"); */
     /* 	     return 1; */
     /* } */
	printk("g_txtick=%x,jiffies=%x\n",g_txtick,jiffies);
    if (jiffies - g_txtick < (90 * HZ)) {
     	     printk("wifi tx active, reject wifi_suspend\n");
	     return 1;
    }
/*    status = wmi_powermode_cmd(ar->arWmi, REC_POWER);
    if (status != A_OK)
	{
		printk("%s:wmi_powermode_cmd Failed(status = %d).\n", __func__,status);
	}*/
    ...
    while (ar->arTxPending[ar->arControlEp]) {
	    long timeleft = wait_event_interruptible_timeout(arEvent,
							     ar->arTxPending[ar->arControlEp] == 0,
							     .5 * HZ);
	    if (!timeleft || signal_pending(current)) {
		    /* what can I do? wow resume at once */
		    printk("Fail to setup wifi suspend\n");
	    } else {
		    printk("Setup wifi suspend  successfully\n");
	    }
	    count--;
	    if (count < 0)
		 {
...
//	int status;
	...
	if (ar == NULL)
	{
     ...
     if (ar->arConnected == TRUE) {
	     printk("wifi in connect state, need NOT wifi_resume op\n");
	     return ;
     }
...
//	netif_carrier_off(ar->arNetDev);
  ...
  /*  status = wmi_powermode_cmd(ar->arWmi, MAX_PERF_POWER);
    if (status != A_OK)
	{
        ...
        printk("%s:wmi_powermode_cmd Success!\n", __func__);
*/
...
int mv88w8686_if_sdio_init_module(void)
{
    ...
    printk("Atheros AR6302 driver (@Rockchip): version (%s)\n", "2.06");

     

    #if (WIFI_GPIO_POWER_CONTROL == 1)
	if (wifi_mmc_host == NULL)  /* There is not a Host controller. */
	{
	    printk("SDIO host isn't ready for now.\n");
	    return -ENODEV;
	}
    printk("wifi_mmc_host 0x%08x\n",wifi_mmc_host);
#endif
     

    a_module_debug_support_init();
     printk("a_module_debug_support_init\n");
...
#if (ANDROID_POWER_SAVE == 1)
    printk(":%s:wifi_ps_status: from %d to %d.\n", __func__, wifi_ps_status, WIFI_PS_AWAKE);
    wifi_ps_status = WIFI_PS_AWAKE;
#endif
    printk("wifi_mmc_host->bus_ops 0x%08x\n",wifi_mmc_host->bus_ops);
    if (wifi_mmc_host->bus_ops == NULL) /* No mmc/card is being attached. */
    {
      printk("NO MMC/card is being attached\n");
   ...
   printk(" WiFi function card has been attached.\n");
#endif
    return 0;
}
...
extern void ReadMainMem(unsigned char  addr,unsigned char  *pt,unsigned char count);
extern void WriteMainMem(unsigned char addr,unsigned char *pt,unsigned char count);
                        ...
                        A_UINT8 assocRespLen, A_UINT8 *assocInfo, A_UINT16 protocolReasonStatus)
{
   ...
   if (((reason == NO_NETWORK_AVAIL) && (protocolReasonStatus == 0x0))
    || (ar->arConnected && (reason == DISCONNECT_CMD)))
       {
 //     AR_DEBUG_PRINTF("Tell firmware disconnect\n");
...
#ifdef WAPI_ENABLE
    ar->arWapiEnable = 0;//fix bug EV74805, if recv disconnect event, clear it.
#endif
