...
static struct ad799x_state* ad799x_st = NULL;
int sep0611_read_ad7997(int channel)
{
	u8 cmd;
	u16 data;
	int ret = 0;

	if((ad799x_st == NULL)||(channel<1)||(channel>8))
		return -1;
	
	cmd =AD7997_8_READ_SINGLE|((channel-1)<<AD799X_CHANNEL_SHIFT);
	ret = ad799x_i2c_read16(ad799x_st, cmd, &data);

	if(ret <0)
		return -1;

	//printk("channel (%d) -> data [%d]\n", (data & 0x7000)>>12, (data & 0xfff)>>2);
	
	// ¼ì²éÍ¨µÀ
	if(data & 0x7000 != ((channel-1)<<12))
	{
		return -1;
	}
...
EXPORT_SYMBOL(sep0611_read_ad7997);
				   ...
				   const struct i2c_device_id *id)
{
	...
	ret = ad799x_set_scan_mode(st, 0x30);
	if (ret)
