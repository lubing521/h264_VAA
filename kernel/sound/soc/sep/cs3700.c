...
static int cs3700_set_audio_output(struct snd_soc_codec *codec)
{
	...
	cs3700_write_reg(0x02, 0xC180 | 0xC1AA);	//??????????????is a bug! bit[8] and bit[7] must enable!!!
	...
	cs3700_write_reg(0x33, 0x0139);				//enable (old mute LOUT1)
	cs3700_write_reg(0x34, 0x0139);				//3db

	cs3700_write_reg(0x35, 0x0079);				//
	cs3700_write_reg(0x36, 0x0079);				//
