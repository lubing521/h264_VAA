...
static void getPictureName (char *Picture, int fmt)
{
	...
	*/
	...
	snprintf (temp, 16, "q_%05d.%s", index++, myext[fmt]);
...
//	unsigned char *picture = NULL;
	...
	if(name){
		...
		//fprintf (foutpict, "P6\n%d %d\n255\n", width, height);
		fwrite (buf, sizeof (char), width * height * 2, foutpict);
		fclose (foutpict);
		free(name);
	}
//	free(picture);
