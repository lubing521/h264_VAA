...
void BlowfishEncrption(unsigned long *szMessage,int len)
{
    int i;
    for (i=0;i<len;i++)
        printf("before %s  v[%d] is %lx\n",__FUNCTION__,i,szMessage[i]);
    for (i=0;i<len;i+=2)
    {
        BlowfishEncipher(&szMessage[i],&szMessage[i+1]);
    }
    for (i=0;i<len;i++)
        printf("after %s  v[%d] is %lx\n",__FUNCTION__,i,szMessage[i]);
}
...
{
    char szKey[]="seuic";
    unsigned long szMessage[4]={1,2,3,4};
    BlowfishKeyInit(szKey,strlen(szKey));
    BlowfishEncrption(szMessage,sizeof(szMessage)/4);
    BlowfishDecrption(szMessage,sizeof(szMessage)/4);
    return 0;
}
