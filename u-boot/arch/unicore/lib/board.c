...
static int display_banner (void)
{
	printf ("\n\n%s\n\n", version_string);
	printf (" ************************************\n");
	printf (" **       SEP0611 BOOTLOADER       **\n");
	printf (" **             V1.0               **\n");
	printf (" **     (c)Copyright SEUIC Inc.    **\n");
	printf (" ************************************\n\n");
...
{
#ifndef CONFIG_SEP0611_TB
	//power hold	
	*(volatile unsigned int*)(GPIO_AOPORT_DIR)  &= ~(7);
	*(volatile unsigned int*)(GPIO_AOPORT_SEL)  = (7);
	*(volatile unsigned int*)(GPIO_AOPORT_DATA) |= (7);
#endif
	...
	*(volatile unsigned int*)GPIO_PORTB_SEL &= ~(0xFFFF<<0);  // for Ethernet
	...
	*(volatile unsigned int*)GPIO_PORTA_SEL &= ~(0x17<<12);  //OE WE CS
	/*General purpose GPIO for DM9000, .*/
	*(volatile unsigned int*)GPIO_PORTC_SEL |= (0x1<<7);     //RESET
